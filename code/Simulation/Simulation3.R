...
### "Simulation 3" is step further from Simulation 2 in adapting to work on the results in the simulation study 3 (Section 6.3).
...
ProjectName <- paste0("ZI2000_Sim6_",range[1],"-",range[2])
...
Simbin_gam <- foreach(var2=c(1:2)) %:%  foreach(var1=range[1]:range[2]) %dopar% {
  start <- proc.time()
  datasim <- dataSim6(seedid= var1, nsample = 2000, beta1 = c(-0.7,0.7), beta2 = c(1,-0.5), alphaplus = -1, alphaminus = -1)
  result <-  Mainfunction_concpp(Ystar = datasim$Ystar,
                                 ...
                                 beta1 = c(-0.5,0.5), beta2 = c(0,0),    # was c(1.2,-1.5) for 1-240
                                 ...
                                 seed = var1, nmcmc = 6000)   # , error = function(e) NULL)
  end <- proc.time() - start
